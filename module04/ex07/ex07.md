...
* `compare_box_plots(categorical_var, numerical_var)` : displays in a figure box plots which allow to compare the distribution of the numerical variable according to the categorical values of categorical_var. There should be as many box plots as categories. For example, with Sex and Height, we would compare the height distributions of men vs. women via two box plots.
