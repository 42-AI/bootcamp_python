...
\begin{42console}
function set\_conda \{
	HOME=\textbackslash\{\}$(echo \textasciitilde\{\})
	INSTALL\_PATH="/goinfre"
	MINICONDA\_PATH=\textbackslash\{\}$INSTALL\_PATH"/miniconda3/bin"
	PYTHON\_PATH=\textbackslash\{\}$(which python)
	SCRIPT="Miniconda3-latest-MacOSX-x86\_64.sh"
	REQUIREMENTS="jupyter numpy pandas"
	DL\_LINK="https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-x86\_64.sh"

	if echo \textbackslash\{\}$PYTHON\_PATH | grep -q \textbackslash\{\}$INSTALL\_PATH; then
		echo "good python version :)"
	else
	cd
	if [ ! -f \textbackslash\{\}$SCRIPT ]; then
		curl -LO \textbackslash\{\}$DL\_LINK
		fi
		if [ ! -d \textbackslash\{\}$MINICONDA\_PATH ]; then
			sh \textbackslash\{\}$SCRIPT -b -p \textbackslash\{\}$INSTALL\_PATH"/miniconda3"
	fi
	conda install -y \textbackslash\{\}$(echo \textbackslash\{\}$REQUIREMENTS)
	clear
	echo "Which python:"
	which python
	if grep -q "\textasciicircum\{\}export PATH=\textbackslash\{\}$MINICONDA\_PATH" ~/.zshrc
	then
		echo "export already in .zshrc";
	else
		echo "adding export to .zshrc ...";
		echo "export PATH=\textbackslash\{\}$MINICONDA\_PATH:\textbackslash\{\}\textbackslash\{\}$PATH" >> ~/.zshrc
	fi
	source ~/.zshrc
	fi
\}
...
This should be accomplished by writing a function that takes 2 numbers as parameters and returns 5 values, as formatted 
